...
namespace
{
// Data-object (de)serialization is implemented in VTK::IOLegacy and installed
// via vtkCommunicator::RegisterMarshalDataObjectHandlers() when that module is
// loaded. VTK::ParallelCore keeps no dependency on the IO modules.
vtkCommunicator::MarshalDataObjectHandler vtkCommunicatorMarshalHandler = nullptr;
vtkCommunicator::UnMarshalDataObjectHandler vtkCommunicatorUnMarshalHandler = nullptr;
}
...
//------------------------------------------------------------------------------
void vtkCommunicator::RegisterMarshalDataObjectHandlers(
  MarshalDataObjectHandler marshal, UnMarshalDataObjectHandler unmarshal)
{
  vtkCommunicatorMarshalHandler = marshal;
  vtkCommunicatorUnMarshalHandler = unmarshal;
}

//------------------------------------------------------------------------------
int vtkCommunicator::MarshalDataObject(vtkDataObject* object, vtkCharArray* buffer)
{
  if (vtkCommunicatorMarshalHandler == nullptr)
  {
    vtkGenericWarningMacro("Cannot marshal data object: no serialization handler is registered. "
                           "Load VTK::IOLegacy (e.g. `import vtkmodules.vtkIOLegacy`) to enable "
                           "vtkCommunicator data-object serialization.");
    return 0;
  }
  return vtkCommunicatorMarshalHandler(object, buffer);
}
...
{
  if (vtkCommunicatorUnMarshalHandler == nullptr)
  {
    vtkIdType bufferSize = buffer ? buffer->GetNumberOfTuples() : 0;
    if (bufferSize <= 0)
    {
      // An empty buffer legitimately deserializes to no object; no handler needed.
      return nullptr;
    }
    vtkGenericWarningMacro("Cannot unmarshal data object: no serialization handler is registered. "
                           "Load VTK::IOLegacy (e.g. `import vtkmodules.vtkIOLegacy`) to enable "
                           "vtkCommunicator data-object deserialization.");
    return nullptr;
  }
  return vtkCommunicatorUnMarshalHandler(buffer);
}
