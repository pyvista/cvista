...
# --- cvista drop-in shim -------------------------------------------------------
# Stock VTK's wrapped classes report a ``vtkmodules.*`` ``__module__``; the cvista
# fork builds them under ``cvista.*``. Downstream libraries key behavior on this
...
# other ``__module__``-keyed consumer) treats cvista exactly like stock VTK. This
...
# ``cvista.*`` so pickling/imports still resolve, and the overrides stay
...
for _vtk_dm_cls in (
    ...
    if _vtk_dm_cls.__module__.startswith("cvista."):
        ...
        with suppress(TypeError):
            _vtk_dm_cls.__module__ = "vtkmodules." + _vtk_dm_cls.__module__[len("cvista.") :]
del _vtk_dm_cls
...
class DataSet(object):
    ...
    def convert_to_unstructured_grid(self):
        from cvista.vtkFiltersCore import vtkExtractCells
...
with suppress(ImportError):
    import copyreg
    from cvista.util.pickle_support import serialize_VTK_data_object
