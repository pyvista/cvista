...
def _cell_derivatives(array, dataset, attribute_type, filter):
    from cvista.vtkFiltersCore import vtkCellDataToPointData
...
def _cell_quality(dataset, quality):
    from cvista.vtkFiltersVerdict import vtkCellQuality
...
def _matrix_math_filter(array, operation):
    from cvista.vtkFiltersVerdict import vtkMatrixMathFilter
...
def _curl(array, dataset=None):
    from cvista.vtkFiltersGeneral import vtkCellDerivatives
...
def _volume(dataset):
    from cvista.vtkFiltersVerdict import vtkCellSizeFilter
...
def _surface_normal(dataset):
    from cvista.vtkFiltersCore import vtkPolyDataNormals
