# cvista MINIMAL profile — the lightest possible PyVista-parity VTK.
#
...
#
# Configure with:  cmake -S <src> -B <build> -C cvista-config/minimal.cmake ...
...
# Defines CVISTA_NOWRAP_CLASSES; the hook in CMake/vtkModule.cmake demotes each
...
# --- CVISTA_KEEP_CLASSES: opt-in re-include of trimmed classes ------------------
...
set(CVISTA_KEEP_CLASSES "" CACHE STRING
    "cvista: class names to re-include even if present in the NOWRAP/NOCOMPILE drop lists")
if (CVISTA_KEEP_CLASSES)
  if (CVISTA_NOCOMPILE_CLASSES)
    list(REMOVE_ITEM CVISTA_NOCOMPILE_CLASSES ${CVISTA_KEEP_CLASSES})
  ...
  if (CVISTA_NOWRAP_CLASSES)
    list(REMOVE_ITEM CVISTA_NOWRAP_CLASSES ${CVISTA_KEEP_CLASSES})
    set(CVISTA_NOWRAP_CLASSES "${CVISTA_NOWRAP_CLASSES}"
        CACHE INTERNAL "cvista: classes to skip Python-wrapping (pyvista-unused, fully closed)" FORCE)
  endif ()
endif ()
...
# CVISTA_SOURCE_UNITY (default ON here; CVISTA_SOURCE_UNITY=0 disables). It excludes
# vtkCommonCore (owned by the array-instantiation TU split, PR #27) plus any
# module listed in CVISTA_SOURCE_UNITY_EXCLUDE that does not batch clean. The env
# knob is read at hook time; we default it ON for the production build by exporting
# it from build-cvista.sh, but ALSO record the intended default + the exclude list
# here so a direct `-C minimal.cmake` configure that sets CVISTA_SOURCE_UNITY=1 in
# its env behaves identically.
include("${CMAKE_CURRENT_LIST_DIR}/_source_unity_exclude.cmake")
set(CVISTA_SOURCE_UNITY_BATCH 8 CACHE STRING "cvista: module source .cxx per unity TU")

# --- Lever: Python stable-ABI (abi3 / Py_LIMITED_API) — DEFAULT ON ------------
# When ON (the default), the Python wrapper TUs (generated *Python.cxx +
# WrappingPythonCore) are compiled with Py_LIMITED_API set to CVISTA_ABI3_VERSION
...
# generator are fully ported to PyType_FromSpec heap types behind CVISTA_ABI3; the
...
# backend (ci/cibw/cvista_backend.py) FORCES CVISTA_ABI3 OFF on the cp311 leg (3.11
...
#
# ESCAPE HATCH: configure with -DCVISTA_ABI3=OFF (or CVISTA_ABI3=0 in the wheel
...
option(CVISTA_ABI3 "cvista: compile Python wrappers against the CPython stable ABI (Py_LIMITED_API). Default ON (3.12+); the cibuildwheel backend forces OFF on cp311 (static wheel). See docs/abi3-feasibility.md" ON)
...
set(CVISTA_ABI3_VERSION "0x030c0000" CACHE STRING "cvista: Py_LIMITED_API value when CVISTA_ABI3 is ON (0x030c0000 = CPython 3.12 floor)")

# --- build hygiene -----------------------------------------------------------
set(CMAKE_BUILD_TYPE "Release" CACHE STRING "")

# --- Lever: -O2 for the PR gate only (CVISTA_GATE_O2=1) ------------------------
...
#
# Gate-only: enabled via the env knob CVISTA_GATE_O2=1 (set in the PR-gate
# CIBW_ENVIRONMENT alongside CVISTA_LTO=0). Release builds leave it unset -> -O3.
...
if ("$ENV{CVISTA_GATE_O2}" STREQUAL "1")
...
# know otherwise. ci/cmake/windows.cmake sets -DCVISTA_FORCE_MSVC=ON and
# ci/cmake/macos.cmake sets CVISTA_TARGET_OS=macos / runs on an APPLE host so the
# first-pass detection still fires; the MSVC/WIN32/compiler-id checks catch the
# post-project() re-read as belt-and-suspenders.
set(_CVISTA_TOOLCHAIN "gnu")
if (CVISTA_FORCE_MSVC OR MSVC OR WIN32 OR CMAKE_CXX_COMPILER_ID MATCHES "MSVC")
  set(_CVISTA_TOOLCHAIN "msvc")
elseif (CMAKE_HOST_APPLE OR "$ENV{CVISTA_TARGET_OS}" STREQUAL "macos")
...
# Free and universally safe on glibc (the only cvista target); ~0.27 MiB measured.
...
if (_CVISTA_TOOLCHAIN STREQUAL "gnu")
  set(_cvista_ldflags "-Wl,--gc-sections -Wl,--hash-style=gnu")
elseif (_CVISTA_TOOLCHAIN STREQUAL "apple")
  ...
  set(_cvista_ldflags "-Wl,-dead_strip")
else ()
...
set(CMAKE_SHARED_LINKER_FLAGS "${CMAKE_SHARED_LINKER_FLAGS} ${_cvista_ldflags}" CACHE STRING "" FORCE)
set(CMAKE_MODULE_LINKER_FLAGS "${CMAKE_MODULE_LINKER_FLAGS} ${_cvista_ldflags}" CACHE STRING "" FORCE)
set(CMAKE_EXE_LINKER_FLAGS    "${CMAKE_EXE_LINKER_FLAGS} ${_cvista_ldflags}" CACHE STRING "" FORCE)
unset(_cvista_ldflags)
...
# CVISTA_SEMINTERP=0. clang accepts -fno-semantic-interposition too, so gnu+apple
...
if (NOT _CVISTA_TOOLCHAIN STREQUAL "msvc" AND NOT "$ENV{CVISTA_SEMINTERP}" STREQUAL "0")
...
# time -O3 governs), which is fine under a speed-first mandate. Toggle CVISTA_LTO=0
# for fast iteration builds.
if (NOT "$ENV{CVISTA_LTO}" STREQUAL "0")
  if (_CVISTA_TOOLCHAIN STREQUAL "gnu")
    ...
    set(_cvista_lto_link "-flto=auto -O3 -fno-semantic-interposition")
    set(CMAKE_SHARED_LINKER_FLAGS "${CMAKE_SHARED_LINKER_FLAGS} ${_cvista_lto_link}" CACHE STRING "" FORCE)
    set(CMAKE_MODULE_LINKER_FLAGS "${CMAKE_MODULE_LINKER_FLAGS} ${_cvista_lto_link}" CACHE STRING "" FORCE)
    set(CMAKE_EXE_LINKER_FLAGS    "${CMAKE_EXE_LINKER_FLAGS} ${_cvista_lto_link}" CACHE STRING "" FORCE)
    unset(_cvista_lto_link)
  elseif (_CVISTA_TOOLCHAIN STREQUAL "apple")
    ...
    set(_cvista_lto_link "-flto=thin")
    set(CMAKE_SHARED_LINKER_FLAGS "${CMAKE_SHARED_LINKER_FLAGS} ${_cvista_lto_link}" CACHE STRING "" FORCE)
    set(CMAKE_MODULE_LINKER_FLAGS "${CMAKE_MODULE_LINKER_FLAGS} ${_cvista_lto_link}" CACHE STRING "" FORCE)
    set(CMAKE_EXE_LINKER_FLAGS    "${CMAKE_EXE_LINKER_FLAGS} ${_cvista_lto_link}" CACHE STRING "" FORCE)
    unset(_cvista_lto_link)
...
# Speed-3: Profile-Guided Optimization (CVISTA_PGO=gen | use). Two-phase: an
...
#        phase with CVISTA_LTO=0 CVISTA_ICF=0 (instrument cleanly; ICF would fold
...
#        CVISTA_LTO=1 CVISTA_ICF=1 for the shipped wheel (profile guides LTO inlining).
if (NOT _CVISTA_TOOLCHAIN STREQUAL "gnu" AND NOT "$ENV{CVISTA_PGO}" STREQUAL "")
  ...
  # out of scope for the first macOS/Windows wheels (built with CVISTA_PGO unset).
  message(WARNING "CVISTA_PGO is GCC-only; ignored on the ${_CVISTA_TOOLCHAIN} build.")
elseif ("$ENV{CVISTA_PGO}" STREQUAL "gen")
  set(_cvista_pgo "-fprofile-generate -fprofile-update=atomic")
  set(CMAKE_C_FLAGS   "${CMAKE_C_FLAGS} ${_cvista_pgo}" CACHE STRING "" FORCE)
  set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} ${_cvista_pgo}" CACHE STRING "" FORCE)
  ...
  unset(_cvista_pgo)
elseif ("$ENV{CVISTA_PGO}" STREQUAL "use")
  set(_cvista_pgo "-fprofile-use -fprofile-correction -Wno-coverage-mismatch")
  set(CMAKE_C_FLAGS   "${CMAKE_C_FLAGS} ${_cvista_pgo}" CACHE STRING "" FORCE)
  set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} ${_cvista_pgo}" CACHE STRING "" FORCE)
  unset(_cvista_pgo)
endif ()
...
# linker (-fuse-ld=…):
#
#   CVISTA_FAST_LINKER — which ELF linker drives every link edge. The link phase
...
#
#   CVISTA_ICF — Identical Code Folding (--icf=all). Folds byte-for-byte identical
...
#   CVISTA_ICF=0 for an A/B baseline or if a function-pointer-identity issue
...
# -DCVISTA_*=… cache var would be too late to gate the appends below (and
# CMAKE_SYSTEM_NAME isn't set yet either). $ENV{} is available now and matches
# the repo's env-knob idiom (CVISTA_STRIP, CVISTA_LTO, FAST).
#
...
if(_CVISTA_TOOLCHAIN STREQUAL "gnu")
  ...
  if (NOT DEFINED ENV{CVISTA_FAST_LINKER})
    set(_cvista_want "mold")
  else ()
    set(_cvista_want "$ENV{CVISTA_FAST_LINKER}")
  endif ()
  if (_cvista_want STREQUAL "off")
    set(_cvista_ld "")                      # compiler default linker (bfd)
  elseif (_cvista_want MATCHES "^(mold|lld|gold)$")
    set(_cvista_ld "${_cvista_want}")
  else ()
    message(WARNING "CVISTA_FAST_LINKER='${_cvista_want}' unrecognized "
  ...
  if (NOT _cvista_ld STREQUAL "")
    # mold ships as `mold`/`ld.mold`; lld as `ld.lld`; gold as `ld.gold`.
    if (_cvista_ld STREQUAL "mold")
      find_program(_cvista_ld_bin NAMES mold ld.mold)
    else ()
      find_program(_cvista_ld_bin NAMES "ld.${_cvista_ld}")
    endif ()
    if (NOT _cvista_ld_bin)
      find_program(_cvista_gold_bin NAMES ld.gold)
      if (_cvista_gold_bin AND NOT _cvista_ld STREQUAL "gold")
        message(STATUS "CVISTA_FAST_LINKER: '${_cvista_ld}' not found on PATH; "
                       "falling back to gold.")
        set(_cvista_ld "gold")
      elseif (NOT _cvista_gold_bin)
        message(STATUS "CVISTA_FAST_LINKER: '${_cvista_ld}' (and gold) not found on "
                       "PATH; using the compiler default linker (bfd).")
        set(_cvista_ld "")
      endif ()
      unset(_cvista_gold_bin CACHE)
    endif ()
    unset(_cvista_ld_bin CACHE)
  endif ()
  if (NOT _cvista_ld STREQUAL "")
    set(_cvista_link "${_cvista_link} -fuse-ld=${_cvista_ld}")
  endif ()
  unset(_cvista_want)
  unset(_cvista_ld)
  # --- ICF (default ON) ---
  if (NOT DEFINED ENV{CVISTA_ICF} OR NOT "$ENV{CVISTA_ICF}" STREQUAL "0")
    set(_cvista_link "${_cvista_link} -Wl,--icf=all")
  endif ()
  if (NOT "${_cvista_link}" STREQUAL "")
    set(CMAKE_SHARED_LINKER_FLAGS "${CMAKE_SHARED_LINKER_FLAGS} ${_cvista_link}" CACHE STRING "" FORCE)
    set(CMAKE_MODULE_LINKER_FLAGS "${CMAKE_MODULE_LINKER_FLAGS} ${_cvista_link}" CACHE STRING "" FORCE)
    set(CMAKE_EXE_LINKER_FLAGS    "${CMAKE_EXE_LINKER_FLAGS} ${_cvista_link}" CACHE STRING "" FORCE)
  endif ()
  unset(_cvista_link)
endif()
...
# CVISTA_LTO=0 separately). No PGO/--hash-style/-fno-semantic-interposition analogue
# on PE/COFF (see the comments on those levers above).
if (_CVISTA_TOOLCHAIN STREQUAL "msvc")
  set(CMAKE_C_FLAGS   "${CMAKE_C_FLAGS} /Gy /Gw" CACHE STRING "" FORCE)
  set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} /Gy /Gw" CACHE STRING "" FORCE)
  set(_cvista_msvc_link "/OPT:REF /OPT:ICF")
  set(CMAKE_SHARED_LINKER_FLAGS "${CMAKE_SHARED_LINKER_FLAGS} ${_cvista_msvc_link}" CACHE STRING "" FORCE)
  set(CMAKE_MODULE_LINKER_FLAGS "${CMAKE_MODULE_LINKER_FLAGS} ${_cvista_msvc_link}" CACHE STRING "" FORCE)
  set(CMAKE_EXE_LINKER_FLAGS    "${CMAKE_EXE_LINKER_FLAGS} ${_cvista_msvc_link}" CACHE STRING "" FORCE)
  unset(_cvista_msvc_link)

  # === Windows abi3 (Py_LIMITED_API) link fixes ================================
  # PART 1 (LNK1104 "cannot open file 'python3.lib'") is handled OUTSIDE cmake:
  # the backend (ci/cibw/cvista_backend.py::_set_abi3_windows_libdir) prepends the
...
if (_CVISTA_TOOLCHAIN STREQUAL "apple")
...
elseif (_CVISTA_TOOLCHAIN STREQUAL "msvc")
  set(VTK_OPENGL_HAS_EGL    False CACHE BOOL "")
  set(VTK_OPENGL_HAS_OSMESA False CACHE BOOL "")
...
# Independent `cvista` distribution: the Python package installs as `cvista/` (not
# `vtkmodules/`) and the wheel's dist name is `cvista` (set in setup.py.in), so it
