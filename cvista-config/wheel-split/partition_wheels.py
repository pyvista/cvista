...
def main(rootdir, outdir):
    ...
    # ---- module->tier table for the flat namespace -----------------------------
    # cvista/__init__.py's flat __getattr__ resolves any class of the FULL build
    # via the generated _class_index.py (scaffolding -> rides core). When the
    # hosting module is absent on a partial install, it consults this table to
    # raise "pip install cvista[<tier>]" instead of a bare error. Emitted here --
    # not at build time -- because THIS script owns tier assignment.
    core_pkg = Path(outdir) / "core" / "cvista"
    if not (core_pkg / "_class_index.py").exists():
        print("ERROR: cvista/_class_index.py missing from the core tree -- the "
              "build did not generate the flat class index (CVISTA_CLASS_INDEX)")
        sys.exit(1)
    with open(core_pkg / "_module_tiers.py", "w", encoding="utf-8", newline="\n") as fh:
        fh.write('"""Module -> tier-wheel table.  GENERATED by partition_wheels.py -- do not edit.\n\n'
                 'Consulted by the flat namespace in __init__.py to name the tier wheel that\n'
                 'ships a module absent from a partial install.\n"""\n\nTIERS = {\n')
        for m in sorted(module_names):
            fh.write(f"    {m!r}: {module_tier(m)!r},\n")
        fh.write("}\n")

    # ---- shared-lib -> tier table for the flat namespace -----------------------
    # A compiled module in one tier may link a shared library that ships in ANOTHER
    # (allowed) tier -- a rendering scene-exporter links io's libvtkIO, say. On a
    # partial install (rendering without io) that module is PRESENT but fails to
    # load because the dependency lib is absent. The flat __getattr__ parses the
    # missing library out of the load error and consults this table to name the
    # tier to install, which differs from the module's own tier. Keys are the
    # platform-independent lib stem (no lib-prefix, no .so/.dylib/.dll/.abi3 suffix,
    # no version) so one table serves every platform's error phrasing.
    def _lib_stem(fname):
        s = fname[3:] if fname.startswith("lib") else fname
        return re.sub(r'\.(abi3\.)?(so|dylib|dll|pyd)(\.\d+)*$', '', s)

    lib_tiers = {}
    for fname, t in sorted(tier_of_name.items()):
        lib_tiers.setdefault(_lib_stem(fname), t)
    with open(core_pkg / "_lib_tiers.py", "w", encoding="utf-8", newline="\n") as fh:
        fh.write('"""Shared-lib stem -> tier-wheel table.  GENERATED by partition_wheels.py -- '
                 'do not edit.\n\nConsulted by the flat namespace in __init__.py to name the tier '
                 'whose shared\nlibrary a present-but-unloadable module is missing on a partial '
                 'install.\n"""\n\nLIBS = {\n')
        for stem in sorted(lib_tiers):
            fh.write(f"    {stem!r}: {lib_tiers[stem]!r},\n")
        fh.write("}\n")
    print(f"  wrote {core_pkg / '_lib_tiers.py'}: {len(lib_tiers)} shared libs")
    print(f"  wrote {core_pkg / '_module_tiers.py'}: {len(module_names)} modules")
