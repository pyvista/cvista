...
<p align="center">
  <strong>cvista: a fast, lean fork of VTK, maintained for the PyVista community</strong>
</p>

<p align="center">
  <a href="https://pypi.org/project/cvista/"><img src="https://img.shields.io/pypi/v/cvista.svg?logo=python&logoColor=white" alt="PyPI" /></a>
  <a href="https://pypi.org/project/cvista-sdk/"><img src="https://img.shields.io/pypi/v/cvista-sdk.svg?label=cvista-sdk&logo=python&logoColor=white" alt="cvista-sdk PyPI" /></a>
  <a href="https://pypi.org/project/cvista/"><img src="https://img.shields.io/badge/ABI-abi3%20(CPython%203.12%2B)-orange?logo=python&logoColor=white" alt="Stable ABI (abi3), CPython 3.12+" /></a>
...
<p align="center">
  <em>
    cvista is an open, community-maintained graphics layer for
    <a href="https://github.com/pyvista/pyvista">PyVista</a>. It is BSD-3 licensed,
    developed in the open on <a href="https://github.com/pyvista/cvista">pyvista/cvista</a>,
    and not affiliated with Kitware.
  </em>
</p>

**cvista** is a fork of [VTK](https://gitlab.kitware.com/vtk/vtk)
...
fixes they send, and to ship on a cadence the community can plan around. cvista is the PyVista
...
This is not a hostile fork. cvista respects VTK's BSD license, records its upstream provenance,
...
cvista has two phases. Today it is a trimmed, drop-in VTK. Over time it diverges as we replace
components with faster implementations.

| | cvista | stock `vtk` 9.6.2 |
|---|---|---|
...
| Import name | `cvista` | `vtkmodules` |

**Phase 1, today: a trimmed drop-in.** cvista ships only the modules PyVista imports (core,
filters, IO, and the full rendering stack) and their dependencies. It installs as the `cvista`
...
unchanged once it is taught to import `cvista`.
...
`cvista.EnableFast()` opt-in, so the default build stays an exact drop-in. The contract is
...
The quick path is an import shim that aliases `vtkmodules` to `cvista` before PyVista loads,
...
The intended end state is a PyVista backend selector (for example a `PYVISTA_VTK_BACKEND=cvista`
switch) that imports `cvista` directly. That work lands in PyVista itself. See the
[namespace section](docs/build-internals.md#namespace--coexists-with-stock-vtk) for details.

## Modules beyond the core

cvista's core ships lean on purpose. It carries the module closure PyVista actually uses, and
...
- **cvista is the base.** It builds and publishes both a runtime wheel and a development **SDK**
  ...
  `find_package(VTK)`.
- **Other modules live in their own repos.** A VTK module that cvista does not ship can be
  maintained as a small, focused package that builds against the cvista SDK. It depends on
  cvista, releases on its own schedule, and is owned by the people who care about it.
...
community relies on, building it against the cvista SDK is the path to a release cadence you
control.

## Contributing

cvista is for the PyVista community, and it is built by the PyVista community. Contributions of
...
The one rule that governs everything is the parity contract: **cvista is a bit-exact drop-in for
...
2. **Correct but reorders output → must be opt-in.** It goes behind `cvista.EnableFast()`, with
...
Development happens on [`pyvista/cvista`](https://github.com/pyvista/cvista) via GitHub pull
...
- **A published SDK.** Ship the development SDK (headers, CMake config, import libraries)
  alongside the runtime wheel so external module repos can build against cvista directly.
- **First-class PyVista support.** A PyVista backend selector that imports cvista natively,
  replacing the import shim.
- **More speed.** Continue replacing hot VTK components with faster implementations under the
  parity contract. This is the phase where cvista earns the "f".
...
cvista is a fork of VTK 9.6.2, distributed under VTK's own OSI-approved BSD 3-Clause license
(see [`Copyright.txt`](Copyright.txt)). Upstream provenance is recorded in the root commit.
cvista is not affiliated with or endorsed by Kitware, does not feed changes back to the Kitware's
...
cvista inherits VTK's BSD 3-Clause license; see [`Copyright.txt`](Copyright.txt). For build
