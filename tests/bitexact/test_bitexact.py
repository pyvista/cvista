"""Bit-exactness regression tests: fvtk (this fork) vs stock VTK 9.6.2.
...
Prereqs (see conftest.py): BITEXACT_STOCK_PY / BITEXACT_FVTK_PY pointing at a
stock-VTK python and an fvtk python; tests skip cleanly if unset.
"""
...
def test_provenance_inputs_identical(results):
    ...
    inputs. If this fails, any downstream diff is an input artifact, not fvtk."""
    prov = results["provenance"]
    assert prov["numpy_match"], (
        f"numpy version mismatch stock={prov['numpy_stock']} "
        f"fvtk={prov['numpy_fvtk']}; pin numpy==2.4.6 on both venvs"
    )
    assert prov["inputs_digest_match"], (
        ...
        f"fvtk={prov['inputs_digest_f64_fvtk'][:16]}"
    )
    assert prov["vtk_stock"] == "9.6.2", prov["vtk_stock"]
    assert prov["vtk_fvtk"] == "9.6.2", prov["vtk_fvtk"]
...
def test_bitexact(results, case_key, op_name):
    """Every output array byte-identical between fvtk and stock VTK."""
