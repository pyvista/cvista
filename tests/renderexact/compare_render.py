"""Pixel-exact comparison of two run_render.py output dirs (stock vs fvtk).
...
def compare_case(stock_dir, fvtk_dir, name):
    sp = os.path.join(stock_dir, name + ".npz")
    fp = os.path.join(fvtk_dir, name + ".npz")
...
def compare_all(stock_dir, fvtk_dir):
    ...
    prov = {
        ...
        "gl_renderer_fvtk": gl_f.get("gl_renderer"),
        "gl_version_stock": gl_s.get("gl_version"),
        "gl_version_fvtk": gl_f.get("gl_version"),
        "gl_match": (
    ...
    for name in keys:
        ...
        if "error" in cs or "error" in cf:
            cases[name] = {
                "ok": False,
                "detail": {"reason": "scene errored", "stock": cs.get("error"), "fvtk": cf.get("error")},
            }
            continue
        ok, detail = compare_case(stock_dir, fvtk_dir, name)
        detail["window_class_stock"] = cs.get("window_class")
        detail["window_class_fvtk"] = cf.get("window_class")
...
if __name__ == "__main__":
    ...
    prov = res["provenance"]
    print("== provenance ==")
    print(f"  vtk      stock={prov['vtk_stock']} fvtk={prov['vtk_fvtk']}")
    print(f"  GL_REND  stock={prov['gl_renderer_stock']!r}")
    print(f"           fvtk ={prov['gl_renderer_fvtk']!r}")
    print(f"  GL_VER   stock={prov['gl_version_stock']!r}")
    print(f"           fvtk ={prov['gl_version_fvtk']!r}")
