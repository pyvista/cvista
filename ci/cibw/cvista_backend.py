"""PEP 517 in-tree build backend that drives cvista's cmake -> build -> generated
build-tree ``setup.py bdist_wheel`` flow, so ``pip wheel .`` (and therefore
cibuildwheel) can build the wheel.

cvista is NOT a normal ``pip install .`` project: VTK only emits a ``setup.py``
...
Knobs (env):
  CVISTA_BUILD_DIR        cmake build tree (default <repo>/build-cibw). ONE constant
  ...
  CVISTA_REUSE_TREE       1 = keep the tree between legs and drop only the ABI-
  ...
  CVISTA_BUILD_DIR_PER_ABI  1 = key the tree by SOABI (side-by-side per-leg trees);
                        DEFEATS cross-leg ccache — only for debugging one leg.
  CVISTA_BUILD_JOBS       cmake --build --parallel N (default: os.cpu_count()).
  CVISTA_CMAKE_INIT       init-cache file (default ci/cmake/linux.cmake).
  CMAKE_C/CXX_COMPILER_LAUNCHER  honoured by cmake (e.g. ccache) if exported.
  CVISTA_LTO etc.         consumed by the init-cache exactly as in the raw build.
"""
...
#
# Escape hatch (reversible — re-adds the legacy static path): CVISTA_ABI3=0 in the
...
ABI3_FLOOR_TAG = "cp312"  # mirrors CVISTA_ABI3_VERSION 0x030c0000 in minimal.cmake
...
def _version_suffix() -> str:
    ...
    version from the repo's tags (honouring ``SETUPTOOLS_SCM_PRETEND_VERSION_FOR_CVISTA``
    ...
    try:
        ...
        version = get_version(
            root=REPO,
            dist_name="cvista",  # so SETUPTOOLS_SCM_PRETEND_VERSION_FOR_CVISTA is honoured
        ...
        print(
            f"cvista_backend: setuptools_scm version unavailable ({exc}); "
    ...
    print(f"cvista_backend: scm version {version} -> VTK_VERSION_SUFFIX={suffix}", flush=True)
    return suffix
...
def _build_dir() -> str:
    ...
    #   * legacy static wheel (CVISTA_ABI3=0 escape hatch only): an SOABI-keyed tree
    ...
    # CVISTA_BUILD_DIR overrides the base location.
    #
...
def _jobs() -> str:
    return os.environ.get("CVISTA_BUILD_JOBS", str(os.cpu_count() or 4))


def _init_cache() -> str:
    # CVISTA_CMAKE_INIT selects the per-OS init-cache (ci/cmake/{linux,macos,
    ...
    val = os.environ.get("CVISTA_CMAKE_INIT")
    if not val:
...
def _prepare_tree(build: str) -> None:
    ...
    #
    # CVISTA_REUSE_TREE=1 (opt-in fast path, validate per-toolchain first): keep the
    # tree and only drop the ABI-specific outputs — the wrapper modules in the
    # top-level package dir (they all land in one constant `cvista/`, so distinct
    ...
    if os.environ.get("CVISTA_REUSE_TREE") == "1" and os.path.isdir(build):
        for pkg in ("cvista", "vtkmodules"):
...
def _fix_ccache_basedir() -> None:
    ...
    # Also covers the un-set case (raw build-cvista.sh, local runs).
...
def _configure_and_build():
    ...
    cfg = [
        ...
        # defaults CVISTA_ABI3 ON, and CVISTA_ABI3=0 in the env flips both off.
        f"-DCVISTA_ABI3={'ON' if _abi3_enabled() else 'OFF'}",
...
def _retag_abi3(wheel: str) -> str:
    ...
    print(f"cvista_backend: retagged abi3 wheel -> {os.path.basename(new_path)}", flush=True)
    return new_path
...
def build_wheel(wheel_directory, config_settings=None, metadata_directory=None):
    ...
    print(f"cvista_backend: produced {dest}", flush=True)
    return os.path.basename(wheel)


def build_sdist(sdist_directory, config_settings=None):
    raise RuntimeError(
        "cvista has no sdist: the wheel is generated from the cmake build tree. "
