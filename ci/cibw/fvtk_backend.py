...
def _build_dir() -> str:
    # TWO-WHEEL build trees, each a CONSTANT path so the cross-leg ccache hits.
    #
    ...
    # A build-dir name that varies WITHIN a wheel's leg group changes every C++
    # object's cache key and gives 0% cross-leg hits (measured: varying name ->
    # 0/2 hits; constant name -> full hit on leg 2). So each of the two wheels
    # gets ONE constant tree:
    #   * abi3 wheel (cp312+): ONE "build-cibw-abi3" tree shared by the cp312/313/
    #     314 legs — the wrappers are version-independent under Py_LIMITED_API, so
    #     cibuildwheel's abi3 dedup means only cp312 builds and 313/314 reuse it.
    #   * static cp311 wheel: ONE "build-cibw-cp311" tree (the SOABI-keyed name is
    #     constant for that single leg). Kept distinct from the abi3 tree so the
    #     per-version wrappers don't clobber the abi3 ones.
