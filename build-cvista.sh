#!/usr/bin/env bash
#
# build-cvista.sh — build the FULL 1:1 pyvista-parity cvista wheel.
#
# Source: this VTK checkout (~/source/VTK, v9.6.2).
# Config: cvista-config/{linux,_base,_modules}.cmake — the canonical CoDim parity
...
#
# Two profiles:
#   FAST=1 (default) -> cvista-config/fast.cmake : LTO off, ~25-40 min cold,
#                       minutes warm (ccache). For validation iteration.
#   FAST=0           -> cvista-config/linux.cmake : LTO on, production wheel.
#
...
if [ "${USE_CCACHE:-1}" = "1" ]; then
  ...
  if [ -x "$CCACHE_BIN" ]; then
    export CCACHE_DIR="${CCACHE_DIR:-$HOME/.cache/cvista-ccache}"
...
if [ "${CVISTA_STRIP:-0}" = "1" ]; then
  ...
  if [ -n "$STRIP_BIN" ] && [ -x "$STRIP_BIN" ]; then
    ...
    done < <(find "$BUILD" \( -name '*.so' -o -name '*.so.*' \) 2>/dev/null | grep -E '/cvista/[^/]*\.so(\.[0-9.]+)?$')
...
# missing dirs. KEEP qt: pyvistaqt imports vtkmodules.qt (-> cvista.qt) at import
# time, so dropping it breaks pyvistaqt (gh-142).
for sub in gtk test tk wx; do
  rm -rf "$BUILD/cvista/$sub" "$BUILD"/build/lib.*/cvista/$sub 2>/dev/null || true
done
if [ -f "$BUILD/setup.py" ]; then
  sed -i -E "/'cvista\.(gtk|tk|wx|test)',?/d" "$BUILD/setup.py"
fi
...
# cmake-staged Python wrappers under "$BUILD/cvista"; the KIT libraries
...
# install-valid.
if [ "${CVISTA_STRIP:-0}" = "1" ] && [ -n "${STRIP_BIN:-}" ] && [ -x "${STRIP_BIN:-/nonexistent}" ]; then
