...
if [ "${FVTK_IN_NIX_SHELL:-0}" != "1" ]; then
  exec nix-shell "$REPO/shell.nix" --run "FVTK_IN_NIX_SHELL=1 PROFILE=${PROFILE:-minimal} FAST=${FAST:-1} USE_CCACHE=${USE_CCACHE:-1} FVTK_ICF=${FVTK_ICF:-1} FVTK_STRIP=${FVTK_STRIP:-0} FVTK_WRAP_OPTSIZE=${FVTK_WRAP_OPTSIZE:-1} FVTK_DISPATCH_MINIMAL=${FVTK_DISPATCH_MINIMAL:-1} FVTK_LTO=${FVTK_LTO:-1} FVTK_SEMINTERP=${FVTK_SEMINTERP:-1} FVTK_PGO=${FVTK_PGO:-} FVTK_SOURCE_UNITY=${FVTK_SOURCE_UNITY:-1} FVTK_SOURCE_UNITY_BATCH=${FVTK_SOURCE_UNITY_BATCH:-8} bash '${BASH_SOURCE[0]}'"
fi
