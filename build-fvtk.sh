...
# unstripped wheel carries ~30-40% dead weight. --strip-unneeded preserves the
# DYNAMIC symbol table (exported symbols needed to load/link the module) and
# removes only the regular symtab + debug — safe for runtime, standard for
# release wheels. Strip both the python wrapper .so and the libvtk*.so kits.
if [ "${FVTK_STRIP:-0}" = "1" ]; then
  ...
  if [ -n "$STRIP_BIN" ] && [ -x "$STRIP_BIN" ]; then
    ...
    while IFS= read -r _so; do
      "$STRIP_BIN" --strip-unneeded "$_so" 2>/dev/null && _n=$((_n+1)) || true
    done < <(find "$BUILD/vtkmodules" "$BUILD"/lib* -name '*.so' -o -name '*.so.*' 2>/dev/null | grep -E '\.so(\.[0-9.]+)?$')
    echo "stripped $_n shared objects ($STRIP_BIN --strip-unneeded)"
  else
