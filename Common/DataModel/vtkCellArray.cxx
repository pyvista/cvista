...
// Trimesh (and possibly other consumers) require int64 faces and cast on
// assignment, breaking np.shares_memory() checks in PyVista's test suite
// (issue #118). The per-reader int32 optimisations in vtkPLYReader and
// vtkSTLReader use SetData() directly and are unaffected by this default;
// WidenStorageForValue() guards remain active for 32-bit platforms and for
// callers that explicitly opt into 32-bit storage via SetDefaultStorageIs64Bit.
