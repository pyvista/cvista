...
/**
 * @file   vtkFVTKSMPDefaults.h
 * @brief  fvtk "multicore-by-default" policy for bit-exact-safe filters.
 *
 * This is an fvtk (pyvista/fvtk) addition, not part of upstream VTK.
 *
 * fvtk ships with the global vtkSMPTools backend set to "Sequential", so by
 ...
 * precedence table in vtkFVTKSMPDefaults.cxx):
 ...
 *   - FVTK_SMP_DEFAULT=0 (fvtk env) : turn this default OFF -> everything serial.
 */

#ifndef vtkFVTKSMPDefaults_h
#define vtkFVTKSMPDefaults_h
 ...
 */
constexpr int FVTK_SMP_DEFAULT_THREADS = 4;
 ...
 * the fvtk default-threading policy. If we are already inside a parallel scope,
...
#endif // vtkFVTKSMPDefaults_h
// VTK-HeaderTest-Exclude: vtkFVTKSMPDefaults.h
