...
namespace
{
// True if the FVTK_SMP_DEFAULT env var asks us to turn the default off.
bool DefaultThreadingDisabledByEnv()
{
  const char* v = std::getenv("FVTK_SMP_DEFAULT");
  if (!v)
  {
    return false;
  }
...
//   5. default                     -> STDThread capped at FVTK_SMP_DEFAULT_THREADS.
//
...
vtkSMPTools::Config GetSafeFilterThreadingConfig()
{
  ...
  return vtkSMPTools::Config{ FVTK_SMP_DEFAULT_THREADS, std::string("STDThread"), false };
}
