...
// fvtk opt-in fast polygonal clip: a threaded replacement for vtkClipPolyData's
...
//
// Built only under FVTK_HAVE_OPENMP (mirroring the sibling fast adapters); the
  ...
  double value, int insideOut, vtkPointData* inPD, vtkCellData* inCD, bool copyScalars)
{
  ...
  // (not the parallel pvaClean kernel that fvtkFastCleanPoly uses) for the
...
#else // !FVTK_HAVE_OPENMP -- bit-exact stub, caller runs the stock serial loop.
