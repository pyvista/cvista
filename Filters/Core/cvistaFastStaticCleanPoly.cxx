...
// cvista opt-in fast coincident-point merge for vtkStaticCleanPolyData: VTK
// adapter over the vendored pyvista-algorithms OpenMP point-dedup kernel
// (pvaClean.h, MIT). This TU is built with OpenMP and excluded from the unity
// build so the vendored code stays isolated.
#include "cvistaFastStaticCleanPoly.h"
...
template <typename T>
void BuildOutput(vtkPolyData* input, vtkPolyData* output, int outputPointsPrecision, int inPtType,
  pvu::clean::CleanOutput<T>& out)
{
  ...
  // remove_degenerate_cells=false and the no-collapse gate below, the kernel
  // keeps every poly 1:1 in input order.
  vtkNew<vtkTypeInt32Array> offArr;
  offArr->SetNumberOfValues(static_cast<vtkIdType>(out.offsets.size()));
  std::memcpy(offArr->GetPointer(0), out.offsets.data(), sizeof(std::int32_t) * out.offsets.size());
  ...
  // cell ids ARE the poly ids) -> pass cell data through unchanged, matching
  // stock vtkStaticCleanPolyData when no cell is dropped or converted.
  ...
  int outputPointsPrecision)
{
  ...
  (void)produceMergeMap;
  (void)hasMergingArray;
  (void)outputPointsPrecision;
  return false;
#else
  // Only the exact-merge default regime is accelerated; a real tolerance sphere,
  // point-data averaging, merge-by-data-array, or the PointMergeMap field-array
  // output all fall back to the standard path.
  if (effectiveTolerance != 0.0 || averagePointData || produceMergeMap || hasMergingArray)
  {
    return false;
  }
  // Polys only -- verts/lines/strips each carry their own dedup/convert semantics
  // (and separate cell-data lists) that this path does not reproduce.
  ...
  // polyhedron detection and degenerate conversion (both disabled here); the
  // merge and the connectivity rewrite are driven by offsets, so this is purely
  // to satisfy the kernel's input contract with sane VTK type tags.
  ...
  for (vtkIdType c = 0; c < nPolys; ++c)
  {
    ...
    typeBuf[static_cast<size_t>(c)] = (w == 3 ? VTK_TRIANGLE : (w == 4 ? VTK_QUAD : VTK_POLYGON));
  }
  ...
  if (ptType == VTK_DOUBLE)
  {
    ...
    // vtkStaticCleanPolyData dedups the ids and drops/converts such a poly, which
    // this path does not reproduce, so bail and let the standard path handle it.
